...
description: >
  ...
  doc-code mismatch report, or invokes /tech-docs-mini. Single-repository variant:
  the current working directory IS the repository and docs live INSIDE it
  (docs/tech-docs/ alongside the code). Use tech-docs instead when the workspace
...
under `docs/tech-docs/` from the single git repository at the current working directory.
...
| No flag; `docs/tech-docs/index.md` exists | **Update** — read YAML frontmatter, extract `last-scan` and `last-commit-hash` |
...
Slug format: `{module-name}` in lowercase kebab. If the project name's last segment
equals the module name's first segment, remove the duplicate
(e.g. project `crackling-api` with module `api-host` → slug `api-host`).
...
Paths are relative to the repository root. If two sources contradict each other,

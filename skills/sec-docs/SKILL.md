...
```
.claude/skills/sec-docs/references/entraid/             ← skill-owned (IMMUTABLE)
...
- All files under `.claude/skills/sec-docs/references/entraid/`. These are part of
...
(e.g. `api1`, `api2`, `web`, `app`, `integration`). A project always has at
least one unit; many have several. Each unit may have:
...
Run this step **once per sec-docs invocation**, before any ASVS category that
maps to Entra ID. The step has four phases.

#### Phase 1 — Discover deployable units + app registrations

Detect units. The signals, in order of authority:
...
| Multi-unit repo | `src/<name>/`, `services/<name>/`, `apps/<name>/`, or services in `docker-compose.yml` inside a single repo. |
...
| State | Action |
|---|---|
| Orchestrator file does not exist | Copy `.claude/skills/sec-docs/references/entraid/Invoke-EntraIDAudit.template.ps1` to `docs/sec-docs/reports/entraid/Invoke-EntraIDAudit.ps1`, then replace the manifest block with the discovered manifest. |
...
`.claude/skills/sec-docs/references/entraid/` from a project run. If a check is
...
In a **pipeline run**, the pipeline executes the orchestrator directly (see
`references/azure-pipeline-template.yml`); the JSON will exist when the skill
starts.

In an **interactive run**:
- If `docs/sec-docs/reports/entraid/entraid-audit-latest.json` exists and is no
  more than 7 days older than the oldest repo HEAD timestamp, read it directly.
...
The pipeline in `references/azure-pipeline-template.yml` runs the project
orchestrator (`docs/sec-docs/reports/entraid/Invoke-EntraIDAudit.ps1`) before
the sec-docs skill on every scheduled execution.

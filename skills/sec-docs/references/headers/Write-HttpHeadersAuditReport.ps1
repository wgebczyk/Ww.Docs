...
<#
.SYNOPSIS
    Writes the HTTP Security Headers audit JSON + Markdown reports consumed by sec-docs.

.DESCRIPTION
    IMMUTABLE — part of the sec-docs skill definition.
    Aggregates findings produced by Test-HttpSecurityHeaders.ps1 and persists them
    in a stable schema compatible with the Entra ID audit JSON format.

    Output files in $OutputDir:
      headers-audit-latest.json   — schema consumed by sec-docs
      headers-audit-latest.md     — human-readable summary
      headers-audit-{ts}.json     — timestamped copy for history
      headers-audit-{ts}.md       — timestamped copy for history

.PARAMETER Findings
    Array of finding hashtables (see New-Finding in HttpHeadersAudit.Utils.psm1).

.PARAMETER Endpoints
    Array of hashtables describing the audited endpoints:
        @{ Unit; Environment; BaseUrl; DisplayName }
...
$jsonPath   = Join-Path $OutputDir "headers-audit-$timestamp.json"
$mdPath     = Join-Path $OutputDir "headers-audit-$timestamp.md"
$latestJson = Join-Path $OutputDir "headers-audit-latest.json"
$latestMd   = Join-Path $OutputDir "headers-audit-latest.md"
...
$endpointsDisplay = if (@($Endpoints).Count -gt 0) {
    (@($Endpoints) | ForEach-Object {
        ...
        "- **$unitLabel$envLabel**: ``$($_.BaseUrl)``$nameSfx"
    }) -join "`n"
} else { "_No endpoints configured_" }

$md = @"
# HTTP Security Headers ASVS Audit

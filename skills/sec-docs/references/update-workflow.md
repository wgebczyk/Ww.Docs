...
stack detection (Init Step 5) and include it in all category re-analyses.

## Step 2 — Re-read curated docs

Read all `docs/sec-docs/curated/**/*.md` regardless of changes. Re-index by
category ID and topic name (same as Init Step 7).
...
For each impacted requirement, repeat the Init Step 8c assessment:
...
2. For each Top 10 item: read the (now updated) ASVS category files from
   `docs/sec-docs/pages/` and aggregate findings as in Init Step 9.

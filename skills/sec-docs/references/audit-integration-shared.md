...
Run this step **once per sec-docs invocation per integration**, before any ASVS
category that consumes that integration's evidence.

### Phase 1 — Discover units

Use the discovery table in the integration's `audit-integration.md` to scan the
repos for the entities the orchestrator must audit (app registrations / base URLs /
hostnames / Key Vaults + App Configs / .NET solutions + npm workspaces).
...
- If `docs/sec-docs/reports/{integration}/{integration}-audit-latest.json` exists
  and its modification time is within 7 days of the most recent HEAD commit
  timestamp across the discovered repos (i.e., not stale by more than a week),
  read it directly.

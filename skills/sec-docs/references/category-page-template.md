...
- **{repo-name}** — {technology, framework, runtime version} (source: {repo}/path/to/config)
- **{repo-name}** — {relevant security libraries or middleware} (source: {repo}/path/to/manifest)
...
- ✅ {Observation supporting PASS} (source: {repo}/path/to/file.ext)
- ❌ {Observation supporting FAIL} (source: {repo}/path/to/file.ext)
- ⚠️ {Observation for PARTIAL or ambiguous} (source: {repo}/path/to/file.ext)
...
5. **Citations:** `(source: repo/path/to/file.ext)` — workspace-relative, at end of sentence.

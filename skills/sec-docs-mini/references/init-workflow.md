...
3. Record the HEAD commit hash: `git log -1 --format=%H`.
4. Detect technology stack (see `references/workflow-guide.md`
   Step 5).
5. Discover Entra ID deployable units and app registrations within the repo; create
...
8. Generate OWASP Top 10 2025 report: read
   `references/owasp_top10_2025.md`, pull findings from the 17
   written category files, write `docs/sec-docs/pages/asvs_top10_owasp.md`.
9. Write `docs/sec-docs/index.md` with YAML metadata (`last-commit-hash` from step 3),
   category table, Top 10 row, and Entra ID audit status row.
10. Append Init entry to `docs/sec-docs/log.md`.
11. Run Verification Checklist.
12. Report: categories documented, requirements assessed, curated files incorporated,
    Entra ID audit status, FAIL/PARTIAL counts, Top 10 items covered.

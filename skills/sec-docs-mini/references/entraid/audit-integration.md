...
A "deployable unit" is an independently deployed component (e.g. `api`, `web`,
`worker`, `integration`). The single repository may have one unit or several.
A unit may have zero, one, or multiple app registrations (e.g. API + SPA, or
one per environment).
...
# A project may have several units (api, web, worker, integration, ...).
# A unit may have one or more app registrations across environments.
# Each entry: @{ Unit; Environment; ClientId; DisplayName }
$Applications = @(
    @{ Unit = "api"; Environment = "Dev";  ClientId = "22222222-2222-2222-2222-222222222222"; DisplayName = "api Dev"  }
    @{ Unit = "api"; Environment = "Prod"; ClientId = "33333333-3333-3333-3333-333333333333"; DisplayName = "api Prod" }
...
```powershell
.\docs\sec-docs\reports\entraid\Invoke-EntraIDAudit.ps1 -RepoRoots .
```

...
| State | Action |
|---|---|
| Orchestrator missing | Copy `references/entraid/Invoke-EntraIDAudit.template.ps1` to `docs/sec-docs/reports/entraid/Invoke-EntraIDAudit.ps1`, then replace the manifest block with the discovered manifest. |
...
`references/entraid/` from a project run.

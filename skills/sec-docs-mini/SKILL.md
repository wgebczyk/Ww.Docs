...
description: >
  ...
  or invokes /sec-docs-mini. Performs OWASP ASVS v5 security analysis category by
  category. Single-repository variant: the current working directory IS the repository
  and docs live INSIDE it (docs/sec-docs/ alongside the code). Scans recursively only;
  never walks up to a parent or enumerates sibling repos. Use sec-docs instead when
  the workspace is a docs repo with sibling code repos checked out as first-level
  child directories.
allowed-tools:
  - Read
  - Write
...
This is the **single-repository** variant of `sec-docs`. The workspace **is** the
repository — the skill scans the current working directory and its subtree only.
It does not walk up to the parent folder, and it does not enumerate sibling repos.
...
The project orchestrators each locate the skill utilities via a `.github`
marker walk (`.github/skills/sec-docs-mini/references/{integration}/`), so the
orchestrator file must live below a checkout where `.github/skills/sec-docs-mini/`
exists.

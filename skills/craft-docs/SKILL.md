...
description: >
  ...
  audit clean-code adherence, or audit testability, or invokes /craft-docs. Produces
  a prioritized Critical/Major/Minor findings report across
  Architecture → SOLID → Clean Code → Testability. Multi-repository variant: the
  ...
  craft-docs-mini instead when scanning a single repository where docs live INSIDE
  the repo (docs/craft-docs/ alongside the code).
allowed-tools:
  - Read
  - Write
  - Edit
...
Architecture, SOLID, Clean Code, Testability — for every git repository
discovered at the workspace root. Output is a prioritized
...
Architecture-style files — load **exactly one** per repo/module after
detection:
...
detected) and report layout are defined in `references/output-format.md`.
...
| No flag; `docs/craft-docs/index.md` exists | **Update** — read YAML frontmatter, extract `last-scan` and `repositories[].last-commit-hash` |

For pipeline generation flags, see Pipeline Generation.

## Pipeline Generation

The pipeline YAML is generated from `references/azure-pipeline-template.yml`
and written to `docs/craft-docs/azure-pipeline.yml`.
...
Passing `--gen-pipeline` forces generation and overwrites any existing file.
Use it when new repositories have been added.
...
| `{REPO_N_ALIAS}` | camelCase of repo name (remove hyphens, uppercase next char — e.g. `cold-wave-api` → `coldWaveApi`) |

For repositories beyond the two shown in the template, replicate the
`resources.repositories` entry and the corresponding `checkout` step for each
additional repo.
...
- `docs/craft-docs/azure-pipeline.yml` — created on first Init; overwritten
  only when `--gen-pipeline` is explicitly passed
...
Slug format: `{repo-name}-{module-name}` in lowercase kebab. If the repo
name's last segment equals the module name's first segment, remove the
duplicate (e.g. `cold-wave-api` + `api-host` → `cold-wave-api-host`).
...
Paths are relative to the workspace root. For findings, the `Where` field
uses `repo-name/path/to/file.ext:Lstart-Lend`. Priority claims must be
supported by an explicit reason rooted in code per the matching aspect or
architecture file's rubric.

If two sources contradict each other, write both citations and flag the
conflict:
...
`docs/craft-docs/log.md`. See `references/workflow-guide.md` for
the exact entry format.
...
Findings Quality, Other Reports, Naming, Update-only checks, and Pipeline checks
(when pipeline was generated this run).
